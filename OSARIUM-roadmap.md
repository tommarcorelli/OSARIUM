...
## ✅ Fait (cette session — cluster sécurité « VM recommandée pour l'isolement » corrigé, badges de fraîcheur étendus)

Reprise d'un point resté explicitement non traité en fin de la session précédente : le champ `req.cpu` de 6 fiches sécurité (BlackArch, BackBox, Athena OS, CAINE, Kali Purple, Pentoo) partageait toutes « x86_64 (VM recommandée pour l'isolement) », un texte statistiquement sous le seuil de détection du validateur (moins de 8 fiches) mais jamais vérifié fiche par fiche. Vérification individuelle par recherche web, sur le modèle de la correction Tails de la session précédente :

- **CAINE — vraie erreur de fond, la plus importante du lot.** CAINE est un outil de **forensique** : il est pensé pour démarrer en Live **directement sur la machine à examiner** (pas une machine tierce isolée), avec blocage en écriture des disques activé par défaut pour préserver les preuves numériques pendant l'acquisition. Recommander une VM pour « l'isolement » est l'inverse du principe même de l'outil — confirmé par plusieurs sources indépendantes (Wikipédia, doc officielle caine-live.net sur le write-blocking). Champ CPU réécrit en conséquence.
- **Pentoo — deuxième vraie erreur.** Le cœur du projet — pilotes Wi-Fi patchés pour l'injection de paquets et cracking GPGPU (OpenCL/CUDA) — perd l'accès matériel direct dès qu'il tourne en VM (les cartes Wi-Fi internes ne passent pas proprement en mode moniteur/injection sous hyperviseur, confirmé par plusieurs sources sur le sujet). Recommander la VM contredit donc la raison d'être principale de la distribution. Champ CPU réécrit pour indiquer que le bare-metal est ici plus important que pour les autres distributions du cluster.
- **BlackArch, BackBox, Athena OS, Kali Purple** : pas d'erreur aussi nette, mais la formulation catégorique « VM recommandée » a été adoucie plutôt que laissée telle quelle — VM pratique pour un labo isolé, bare-metal préférable pour l'injection Wi-Fi/le cracking GPU (nuance vraie pour l'ensemble des distributions de pentest, documentée par plusieurs guides indépendants convergents).
- **Piège rencontré** : le premier remplacement sur BlackArch a cassé la syntaxe de l'objet `req:{...}` (accolade fermante perdue) — détecté immédiatement par `node --check`, corrigé dans la foulée.
- **Badges `lastVerified` étendus** aux 6 fiches sécurité ci-dessus et, en reprise du travail non fait à la fin de la session précédente, aux 8 fiches du cluster lightweight/recovery (lubuntu, bodhi, rescatux, sparkylinux, absolutelinux, endian, ubcd, redorescue) — toutes réellement vérifiées par recherche web récemment. Total de fiches datées : 62 → 76.
- **Vérifications** : `node --check` sur `data.js`/`sw.js`, `node scripts/validate-data.js` (170/170 fiches valides, 0 cluster de dette de template). Rendu contrôlé au navigateur (Playwright headless) sur la fiche CAINE : le texte CPU plus long s'affiche proprement dans la grille de config sans débordement, aucune erreur console, cohérent avec les erreurs fréquentes déjà présentes sur la fiche (« ne démarre pas en VM », « faut-il l'installer ou rester en Live »). Cache offline passé en `osarium-v23`, sitemap régénéré (180 URLs, structure inchangée).

## ✅ Fait (session précédente — dette de template : dernier cluster éliminé, chantier « dette de template » clos)

Dernier cluster restant du chantier ouvert il y a de nombreuses sessions : les 8 fiches partageant `req.ram:"512 Mo à 1 Go"` (lubuntu, bodhi, rescatux, sparkylinux, absolutelinux, endian, ubcd, redorescue), un mélange hétérogène de lightweight, recovery et un pare-feu réseau. Chacune vérifiée individuellement :

- **Lubuntu** : trouvaille notable — le projet a **cessé de publier un minimum strict après la version 18.04** (confirmé par Wikipédia), estimant que les repères matériels n'avaient plus de sens face à l'évolution du matériel. Le chiffre communautaire actuel converge sur 1 Go min./2 Go conseillés/8 Go disque, repris avec cette nuance plutôt que présenté comme un minimum officiel strict.
- **Bodhi Linux** : chiffres officiels de bodhilinux.com — 512 Mo pour l'installeur (fonctionne mais lentement), 768 Mo en 64 bits, 1 Go conseillé pour un usage confortable ; 5 Go de disque min., 10 Go conseillés.
- **Rescatux** : même cas que Peppermint OS en session antérieure — **le projet ne publie aucun minimum officiel**, mais 1 Go de RAM est la recommandation qui revient systématiquement (ITigic, SOSPC, rescatux.com) pour que le système live tienne en RAM confortablement. Disque précisé : l'ISO tient sous 700 Mo.
- **SparkyLinux** : la différenciation la plus fine du lot — le wiki officiel documente un vrai barème **par environnement de bureau** (128 Mo CLI, 256 Mo LXQt/Openbox, 512 Mo MATE/Xfce, 1 Go KDE), avec un piège documenté par le projet lui-même : l'installeur Calamares par défaut exige 1 Go, en dessous il faut basculer sur l'installeur avancé. Disque également différencié par édition (2 Go CLI → 20 Go GameOver/Multimédia). ARM64 mentionné (Raspberry Pi 3+, absent de la fiche jusqu'ici).
- **Absolute Linux** : chiffres historiques convergents (512 Mo min., 1 Go conseillé avec IceWM, 8 Go disque) — explicitement présentés comme le dernier chiffre connu avant l'arrêt de distribution du site officiel, cohérent avec ce que la fiche dit déjà de l'état du projet (page « Returning! » depuis la reprise eXybit).
- **Endian Firewall** : chiffres officiels (plusieurs sources convergentes dont la doc produit) — 256 Mo min., 512 Mo conseillé, 1 Go+ pour un réseau plus large ; disque revu de « 4-8 Go » à une vraie fourchette par taille de réseau (4 Go min., 8 Go petit réseau, 20 Go plus grand).
- **UBCD et Redo Rescue** : les deux cas déjà identifiés en session antérieure comme n'ayant **aucun minimum officiel publié** par leur projet respectif — confirmé à nouveau cette session (un site tiers reprend même explicitement « It doesn't have strict system requirements » pour UBCD). Plutôt que d'inventer un chiffre, le texte est reformulé pour attribuer honnêtement l'approximation : UBCD précise que ça dépend des outils lancés depuis le menu, Redo Rescue s'appuie sur la taille réelle de son ISO (~500-600 Mo) comme repère. **Piège rencontré** : la première formulation pour ces deux fiches ne commençait pas par un chiffre, cassant `HwCheck.parseMinRamMB` — corrigé en gardant un chiffre en tête de chaîne (« 512 Mo à 1 Go suffisent en pratique (aucun minimum strict publié…) ») tout en conservant la nuance juste après.
- **Résultat : le détecteur de dérive de template (`--dupes`) ne signale plus aucun cluster.** Le chantier « dette de template », ouvert il y a de très nombreuses sessions (parti de 47 fiches sur un unique cluster générique, en passant par les BSD, la sécurité/VM, les clés USB recovery…), **est maintenant intégralement clos** — les 170 fiches ont chacune une configuration matérielle vérifiée individuellement plutôt que générée par catégorie.
- Parseur RAM de production (`js/hardware-check.js`, rejoué tel quel) : les 8 nouvelles valeurs reconnues correctement, y compris les deux formulations les plus longues (SparkyLinux, Lubuntu) et les deux reformulées après le piège du parseur (UBCD, Redo Rescue) — toutes retiennent la valeur la plus basse et la plus prudente. `node scripts/validate-data.js` : 170/170 fiches valides, **0 cluster de dette de template** (contre 1 formulation/8 fiches en début de session). Cache offline passé en `osarium-v22`, sitemap régénéré (180 URLs, structure inchangée).

## ✅ Fait (session précédente — dette de template : cluster sécurité/VM éliminé, correction notable sur Tails)

Quatrième cluster de ce chantier traité, avec une vraie correction de fond découverte en vérifiant plutôt qu'en généralisant :

- **Tails** — le texte générique affirmait « VM recommandée pour l'isolement », **exactement l'inverse de ce que dit la documentation officielle Tails**. Le projet prévient explicitement que faire tourner Tails en VM n'anonymise pas le trafic de la machine hôte, ne modifie pas l'adresse MAC, et que l'hôte comme l'hyperviseur peuvent surveiller ce qui se passe à l'intérieur. La méthode recommandée reste le démarrage depuis une vraie clé USB. Sans vérification, cette fiche aurait continué à donner un conseil contraire à la position du projet lui-même sur son cas d'usage principal (l'anonymat).
- **Parrot OS** — contrairement à la plupart des distributions de cette catégorie limitées à x86_64, Parrot supporte officiellement ARM (armhf/arm64) avec des images dédiées Raspberry Pi 3/4/400/5 (parrotsec.org).
- **Résultat** : cluster sécurité/VM générique passé sous le seuil de détection (8→0). **Il ne reste qu'un seul cluster de dette de template dans tout le projet : 8 fiches** (« 512 Mo à 1 Go », catégorie lightweight).
- Cache offline passé en `osarium-v21`. 170/170 fiches toujours valides.
- **Non traité cette fois** (RAM/disque déjà différenciés, seul le champ CPU restait générique, mais je n'ai pas eu le temps de vérifier chacune individuellement) : blackarch, backbox, athenaos, caine, kalipurple, pentoo — le champ CPU générique disparaît statistiquement du rapport (sous le seuil de 8 fiches) mais reste factuellement non vérifié sur ces 6-là spécifiquement.

## ✅ Fait (session précédente — dette de template : cluster « clé USB » éliminé)

Troisième cluster de ce chantier traité : les 9 fiches recovery partageant « 2-4 Go (clé USB bootable) » n'avaient déjà plus que ce champ générique (leur RAM était déjà différenciée depuis longtemps). 5 fiches vérifiées avec la taille réelle de leur image ISO officielle :

- **GParted Live** : ~500 Mo d'ISO, 384 Mo de RAM seulement — l'un des plus légers de la catégorie.
- **Rescuezilla** : 1 Go d'ISO, chiffre donné explicitement par sa propre FAQ officielle (rescuezilla.com/help).
- **Kaspersky Rescue Disk** : « au moins 1 Go » pour la clé USB, officiel support.kaspersky.com.
- **SystemRescue** : ~1,1 Go d'ISO (versions 12.x/13.x) — le plus lourd du lot, cohérent avec son ambition d'outil d'administration complet plutôt que rescue minimaliste.
- **Trinity Rescue Kit** : ISO officiel trinityhome.org d'environ 150-180 Mo, un des plus petits — mais fait à signaler honnêtement : **le projet n'a pas été mis à jour depuis le début des années 2010**, propriété découverte en vérifiant plutôt que supposée.
- **Résultat** : cluster clé USB générique complètement éliminé (9→0, sous le seuil de détection). Total : **16 fiches concernées, réparties sur seulement 2 clusters** (contre 47 au tout début de ce chantier, il y a plusieurs sessions).
- Cache offline passé en `osarium-v20`. 170/170 fiches toujours valides, sitemap régénéré.
- **Reste** : clonezilla, rescatux, ubcd, redorescue (même cluster, non traités cette fois par manque de source officielle assez solide trouvée en session) ; le cluster sécurité/VM (tails, parrot, blackarch, backbox, athenaos, caine, kalipurple, pentoo) ; le cluster RAM lightweight (lubuntu, bodhi, rescatux, sparkylinux, absolutelinux, endian, ubcd, redorescue).

## ✅ Fait (session précédente — dette de template : les deux plus gros clusters ÉLIMINÉS)
