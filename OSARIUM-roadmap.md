...
- **Détecteur de dérive de template** ajouté au validateur : il signale toute formulation partagée par 8 fiches ou plus (`node scripts/validate-data.js --dupes` pour le détail). Il expose l'état actuel — 20 formulations pour 134 fiches sur les champs de configuration — présenté à part des avertissements, parce que ces textes sont approximatifs sans être faux. C'est l'inventaire de la dette de génération par catégorie assumée plus bas dans ce document.
